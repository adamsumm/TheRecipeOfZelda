...
class Parser(object):
    ...
    def _parseProduction(self):
        ...
        for vl in lhs.vertices:
            for vr in rhs.vertices:
                if vl.id == vr.id:
                    lhs_to_rhs[vl] = vr
        for el in lhs.edges:
            if el.v1 in lhs_to_rhs and el.v2 in lhs_to_rhs:
                v1 = lhs_to_rhs[el.v1]
                v2 = lhs_to_rhs[el.v2]
                is_good = False
                for er in rhs.edges:
                    if er.v1 == v1 and er.v2 == v2:
                        is_good = True
                        break
                if is_good:
                    lhs_to_rhs[el] = er
        ...
        prune_edges = True    
        if self.lookahead.type == TokenTypes.EXCLAMATION:
            self._match(TokenTypes.EXCLAMATION)
            prune_edges = False
        self.productions.append(Rule(lhs,rhs,lhs_to_rhs,prune=prune_edges))
    ...
    def _parseVertexID(self, token, graph):
        ...
        number = match.group(0) if match is not None else float('nan')
        vertex = graph.find_vertex(number)
        if vertex == None:
